...
if (isMobile == true) {
  ...
  $(document).keydown(function(e) {
    ...
    focusable = form
      .find(
        "input,ons-input,a,select,button,textarea,div[contenteditable=true]"
      )
      .filter(":visible");
...
function Calculation(FoundY) {
  ...
  var FullName = (
    "" +
    document.getElementById("userInputFirstName").value +
    document.getElementById("userInputMiddleName").value +
    document.getElementById("userInputLastName").value
  )
    .toString()
    .toUpperCase();
  ...
  if (monthToDay > Month && dayToDay > Day) {
    var personalYear =
      parseInt(Month) + parseInt(Day) + parseInt(reduce(yearToDay, true));
  } else {
    var personalYear =
      parseInt(Month) + parseInt(Day) + parseInt(reduce(yearToDay - 1, true));
  }
  ...
  var personalYearResult =
    "Personal Year: " + personalYear + "/" + personalYearReduced;
...
function YesVowel(Value) {
  globalSoulNumber += 7;
  findY(Value);
}

function NoConsonant(Value) {
  globalOuterPerson += 7;
  findY(Value);
}

function findY(Value) {
  var ValueLength = parseInt(Value.toString().toUpperCase().length);
  for (i = 0; i < ValueLength; i++) {
    whatsLeft = Value.slice(i + 1);
    var tempValue = Value.charAt(i);
    globalPositionCount += 1;
    if (tempValue == "Y") {
      var foundY = true;
      switch (globalPositionCount) {
        case 1:
          var nth = "st"
          break;
        case 2:
      ...
      var text = "You seam to have a Y in the " + globalPositionCount + nth + " Position of your name. Which sound dose it make?"
      functionConfirm(
        text,
        function Yes() {
          YesVowel(whatsLeft);
...
function convert(Value, FoundY) {
  ...
  OPNWhole += globalOuterPerson;
  SNWWhole += globalSoulNumber;
  PODNWhole += globalOuterPerson;
  PODNWhole += globalSoulNumber
  return {
    OPNWhole: OPNWhole,
    SNWWhole: SNWWhole,
    PODNWhole: PODNWhole
